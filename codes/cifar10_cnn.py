...
x = Conv2D(nb_filters, kernel_size=(3,3), strides=1, padding='same', name='Conv1')(input_tensor)
...
x = Conv2D(nb_filters*2, kernel_size=(3,3), strides=2, padding='same', name='Conv4')(x)
...
history = model.fit_generator(datagen.flow(X_train, y_train, batch_size=16),
                              steps_per_epoch=X_train.shape[0]/16, epochs=4, verbose=1, #5만개인데 배치사이즈만큼 돌리니까 이를 나눠서 이만큼.
